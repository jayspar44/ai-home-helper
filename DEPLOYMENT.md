...
Automated CI/CD using GitHub Actions with Workload Identity Federation (keyless authentication).

**How It Works:**
- Push to `develop` → Auto-deploy to dev environment (no approval)
- Push to `main` → Requires manual approval → Deploy to production
- `develop` automatically syncs with `main` after production deployment

**Workflows:**
- **PR Validation**: Runs ESLint + build checks on PRs
- **Deploy to Development**: Auto-deploys to `https://dev-dot-{project-id}.uc.r.appspot.com`
- **Deploy to Production**: Requires approval, deploys to `https://{project-id}.uc.r.appspot.com`
- **Sync develop with main**: Keeps branches aligned automatically

### Setup & Usage

**First-time setup:** See [GITHUB_ACTIONS_SETUP.md](GITHUB_ACTIONS_SETUP.md) for complete step-by-step instructions (~30 minutes).

**Daily workflow:** Create feature branch → Make changes → PR to `develop` → Auto-deploy to dev → PR to `main` → Approve → Deploy to production

**Manual deployments (fallback):** Your existing npm scripts still work:
```bash
npm run gcp:deploy:dev   # Deploy to dev manually
npm run gcp:deploy:prod  # Deploy to prod manually
```

**Troubleshooting:** See [GITHUB_ACTIONS_SETUP.md](GITHUB_ACTIONS_SETUP.md) for common issues and solutions.
